...
static int setup_mach_mult(void) {
  ...
  if (!(mach_scale.numer & NUMERATOR_MASK)) {
    mach_mult =
        (((uint64_t)mach_scale.numer << HIGH_SHIFT) + mach_scale.denom / 2) /
        mach_scale.denom;
  } else {
    mach_mult = ((((uint64_t)mach_scale.numer << 32) + mach_scale.denom / 2) /
                 mach_scale.denom)
                << EXTRA_SHIFT;
  }
...
int port_clock_gettime(clockid_t clk_id, struct timespec *ts) {
  ...
  if (MPLS_SLOWPATH(!mach_mult))
    mserr = setup_mach_mult();
